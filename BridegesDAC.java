...
/**
 * BridgesAdvanced.java - COMPLETE VERSION WITH DIVIDE AND CONQUER
 *
 ...
 * 6. Sort using DIVIDE AND CONQUER (Merge Sort) - O(n log n).
 * 7. Pick best move.
 */
...
implements MouseListener, MouseMotionListener, KeyListener {
	...
	// Divide and Conquer statistics
	private int mergeSortComparisons = 0;
	private int mergeSortDepth = 0;
	...
	private void generatePuzzle() {
		...
		mergeSortComparisons = 0; mergeSortDepth = 0;
	...
	// ============================================================================
	// DIVIDE AND CONQUER IMPLEMENTATION: MERGE SORT
	// ============================================================================
	
	/**
	 * DIVIDE AND CONQUER STRATEGY: Merge Sort Algorithm
	 * 
	 * This is a classic divide and conquer algorithm that:
	 * 1. DIVIDE: Splits the array into two halves
	 * 2. CONQUER: Recursively sorts each half
	 * 3. COMBINE: Merges the two sorted halves
	 * 
	 * Time Complexity: O(n log n)
	 * Space Complexity: O(n)
	 * 
	 * @param moves - List of Move objects to be sorted
	 * @param depth - Current recursion depth (for statistics)
	 * @return Sorted list of moves (ascending by score)
	 */
	private List<Move> mergeSort(List<Move> moves, int depth) {
		// Update maximum recursion depth reached
		if (depth > mergeSortDepth) {
			mergeSortDepth = depth;
		}
		...
		// Left half: from index 0 to mid (exclusive)
		List<Move> left = new ArrayList<>(moves.subList(0, mid));
		
		// Right half: from index mid to end
		List<Move> right = new ArrayList<>(moves.subList(mid, moves.size()));

		// CONQUER PHASE: Recursively sort both halves
	...
	/**
	 * MERGE operation for Merge Sort (Divide and Conquer)
	 * 
	 ...
	 */
	private List<Move> merge(List<Move> left, List<Move> right) {
		...
		while (i < left.size() && j < right.size()) {
			mergeSortComparisons++; // Count comparison for statistics
	...
	/**
	 * Wrapper method for merge sort that resets statistics
	 * and provides a clean interface for sorting moves.
	 * 
	 * @param moves - List of moves to sort
	 * @return Sorted list (ascending by score)
	 */
	private List<Move> sortMovesUsingDivideAndConquer(List<Move> moves) {
		// Reset statistics
		mergeSortComparisons = 0;
		mergeSortDepth = 0;

		// Perform merge sort (divide and conquer)
		List<Move> sorted = mergeSort(moves, 0);

		// Print statistics for debugging/analysis
		System.out.println("=== DIVIDE AND CONQUER STATISTICS ===");
		System.out.println("Moves sorted: " + moves.size());
		System.out.println("Comparisons: " + mergeSortComparisons);
		System.out.println("Recursion depth: " + mergeSortDepth);
		System.out.println("Expected comparisons (n log n): " + 
			(moves.size() > 0 ? (int)(moves.size() * Math.log(moves.size()) / Math.log(2)) : 0));
		System.out.println("====================================");

		return sorted;
	}

	// --------------- AI CORE LOGIC (7-STEP FLOW WITH DIVIDE AND CONQUER) ----------------

	/**
	 * AI Move Generation using 7-Step Algorithm with Divide and Conquer
	 * 
	 * Step 6 now uses MERGE SORT (Divide and Conquer) for sorting moves
	 */
	private void makeAIMove() {
		if (isSolved()) return;

		// STEP 1: Generate all legal moves (no scoring yet)
		...
		for (int i = 0; i < islands.size(); i++) {
			for (int j = i + 1; j < islands.size(); j++) {
				...
				// Skip if either island is already full
				if (getBridgeCount(a) >= a.required ||
					getBridgeCount(b) >= b.required) continue;

				// Skip if bridge already has max capacity
				...
				// Add as candidate (score will be computed later)
		...
		// STEP 3: Apply Boruvka principle - prefer moves that connect different components
		List<Move> componentMoves = new ArrayList<>();
		for (Move m : candidates) {
			// Check if move connects two different components
			if (!components.get(m.a).equals(components.get(m.b))) {
				componentMoves.add(m);
			}
		}

		// If no cross-component moves, use all candidates
		if (componentMoves.isEmpty()) {
			componentMoves = candidates;
		}

		// STEP 4: Heuristic scoring (Constraint Density Metric)
		for (Move m : componentMoves) {
			// Score = remaining capacity of both islands
			// Lower score = islands closer to completion = better move
			m.score =
				(m.a.required - getBridgeCount(m.a)) +
				(m.b.required - getBridgeCount(m.b));
		}

		// STEP 5 & 6: DIVIDE AND CONQUER - Sort moves using Merge Sort
		List<Move> sortedMoves = sortMovesUsingDivideAndConquer(componentMoves);

		// STEP 7: Select the best move (first element = lowest score)
		Move best = sortedMoves.get(0);
	...
	private void showSolvedPopup() {
		String message = String.format(
			"Puzzle solved successfully!%n%n" +
			"Final Statistics:%n" +
			"Human Score: %d%n" +
			"Computer Score: %d%n%n" +
			"Last AI Move Statistics:%n" +
			"Comparisons: %d%n" +
			"Recursion Depth: %d",
			humanScore, computerScore, mergeSortComparisons, mergeSortDepth
		);
	...
	protected void paintComponent(Graphics g) {
		...
		// Draw Status text with Divide & Conquer statistics
		g2.setColor(Color.DARK_GRAY);
		g2.setFont(new Font("Arial", Font.PLAIN, 11));
		String turnText = aiThinking ? "Computer thinking (Divide & Conquer)..." :
			(currentPlayer == Player.HUMAN ? "Turn: Human" : "Turn: Computer");
		String status = turnText +
				" | H:" + humanScore + " C:" + computerScore +
				" | Comparisons:" + mergeSortComparisons +
				" Depth:" + mergeSortDepth;
	...
	public static void main(String[] args) {
		SwingUtilities.invokeLater(() -> {
			JFrame frame = new JFrame("Bridges - Divide & Conquer");
